...
name = "Brian's brain"   --global name field (required)
...
function isAlive(grid, x, y)
    ...
    if grid[xx][yy] <= 1 then return false else return true end
...
function calculateState(grid, x, y)
    ...
    if grid[x][y] == 2 then return 1 end
    if grid[x][y] == 1 then return 0 end

    if alive == 2 then return 2 end
